...
#ifndef NO_STREAMS
  virtual int read(GA_CPPSTD(istream) & is=GA_CPPSTD(cin));
  virtual int write (GA_CPPSTD(ostream) & os=GA_CPPSTD(cout)) const;
#endif
...
#ifndef NO_STREAMS
  virtual int read(GA_CPPSTD(istream)& is);
  virtual int write (GA_CPPSTD(ostream)& os) const;
#endif
