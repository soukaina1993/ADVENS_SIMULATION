// $Header: /nfs/dsi/cvs/galib/ga/gaconfig.h,v 1.2 1999/12/15 17:39:11 mwall Exp $
/* ----------------------------------------------------------------------------
  config.h
  mbwall 27jun95
...
#if defined(USE_STDH)
#define C_NOT_IN_STD
#define CPP_NOT_IN_STD
#endif
#if defined(C_NOT_IN_STD)
#define GA_CUSE(x)		
#define GA_CSTD(x)		x
#else
#define GA_CUSE(x)		using std::x;
#define GA_CSTD(x)		std::x
#endif
#if defined(CPP_NOT_IN_STD)
#define GA_CPPUSE(x)		
#define GA_CPPSTD(x)		x
#define GA_IOUSE(x)
#else
#define GA_CPPUSE(x)		using std::x;
#define GA_CPPSTD(x)		std::x
#if defined(NO_STREAMS)
#define GA_IOUSE(x)
#else
#define GA_IOUSE(x)		using std::x;
#endif
#endif
/* ----------------------------------------------------------------------------
