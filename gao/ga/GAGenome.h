...
selected to cross with itself, and self-copying is not out of the question)
  ...
  If your genome class defines any new properties you should to define:
       ...
       virtual int read(GA_CPPSTD(istream)&)
       virtual int write(GA_CPPSTD(ostream)&) const
...
evaluator
  ...
  calculated each GA_CSTD(time) it is requested.  This means that any member function
...
#ifndef NO_STREAMS
inline GA_CPPSTD(ostream)& operator<< (GA_CPPSTD(ostream)& os, const GAGenome& genome)
{ genome.write(os); return(os); }
inline GA_CPPSTD(istream)& operator>> (GA_CPPSTD(istream)& is, GAGenome& genome)
