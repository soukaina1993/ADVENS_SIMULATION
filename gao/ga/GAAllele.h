...
/* ----------------------------------------------------------------------------
  ...
  2000 Laboratory for Industrial Energy and System-dgm-epfl
...
public:
  ...
  core(new GAAlleleSetCore<T>(*(set.core))) {wght=set.wght;}
  virtual ~GAAlleleSet(){
    if(core != 0){
      core->cnt -= 1;
  ...
  T weight() const {return wght;}
  T weight (const T& w) {return wght=w;}
...
#ifndef NO_STREAMS
  int read(GA_CPPSTD(istream) &);
  int write(GA_CPPSTD(ostream) & os) const;
#endif
...
protected:
  ...
  T wght;			// how allele is dominant in crossover
};
...
public:
  ...
  T weight(unsigned int pos) const {
     if (pos < sz) return aset[pos]->weight();
     else return aset[sz-1]->weight();
  }
  T weight (unsigned int pos, const T& w) {
     if (pos < sz) return aset[pos]->weight(w);
     else return aset[sz-1]->weight(w);
  }
...
#ifndef NO_STREAMS
template <class T> GA_CPPSTD(ostream)&
operator<< (GA_CPPSTD(ostream)& os, const GAAlleleSet<T> & arg)
{ arg.write(os); return os; }
template <class T> GA_CPPSTD(istream)&
operator>> (GA_CPPSTD(istream)& is, GAAlleleSet<T> & arg)
