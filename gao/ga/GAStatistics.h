...
public:
  ...
  int scores(GA_CPPSTD(ostream)& os, int which=NoScores);
...
inline const char* GAStatistics::scoreFilename(const char* filename){
  ...
  scorefile = 0;
  if(filename){
    scorefile = new char[GA_CSTD(strlen)(filename)+1];
    GA_CSTD(strcpy)(scorefile, filename);
  }
  return scorefile;
}
...
#ifndef NO_STREAMS
inline GA_CPPSTD(ostream)& operator<< (GA_CPPSTD(ostream)& os, const GAStatistics& s)
