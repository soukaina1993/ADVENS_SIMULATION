...
/* ----------------------------------------------------------------------------
 ...
 DESCRIPTION:
  Initialize the random function by using the current GA_CSTD(time) as the seed.
  We use the pid as well as the GA_CSTD(time) for the random seed in an attempt to get
rid of some of the periodicity from the low bits when using only the GA_CSTD(time) as 
the random seed.
